...
public class CinemaInitServiceImpl implements ICinemaInitService {
	...
	public void initVilles() {
		Stream.of("Casablanca","Marrakech","Rabat","Tanger","Agdal").forEach(nameVille->{
	...
	public void initCinemas() {
		...
		villeRepository.findAll().forEach(v->{
			...
			.forEach(nameCinema->{
			 ...
			 cinema.setNombreSalles(2+(int)(Math.random()*5));
			 cinema.setVille(v);
	...
	public void initSalles() {
		...
		cinemaRepository.findAll().forEach(cinema->{
			for (int i = 0; i < cinema.getNombreSalles(); i++) {
				...
				salle.setNombrePlace(5+(int)(Math.random()*8));
	...
	public void initFilms() {
			...
			Stream.of("100%cachmire","parasite","the-NUTCHRACKER")
			.forEach(titreFilm->{
				Film film=new Film();
